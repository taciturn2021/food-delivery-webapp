...
let responseCache = loadCacheFromStorage();
...
// Helper function to store item in cache
...
// Helper function to get item from cache
...
const cleanupCache = () => {
    ...
    Object.keys(responseCache).forEach(key => {
        if (now - responseCache[key].timestamp > CACHE_MAX_AGE) {
...
setInterval(cleanupCache, 120000); // Run cleanup every 2 minutes
