...
const authVerificationLimiter = rateLimit({
    ...
    handler: (req, res, next, options) => {
        ...
        console.warn(`RATE LIMIT EXCEEDED: IP: ${key}, Endpoint: ${req.method} ${req.originalUrl}, Time: ${new Date().toISOString()}`);
        // You could also log to a file or monitoring service here
        res.status(429).json(options.message);
    },
    // Add on successful hit handler to track patterns
    onLimitReached: (req, res, options) => {
        const key = req.ip || 'unknown-ip';
        console.warn(`RATE LIMIT REACHED: IP: ${key}, Endpoint: ${req.method} ${req.originalUrl}, User-Agent: ${req.headers['user-agent'] || 'unknown'}, Time: ${new Date().toISOString()}`);
        // Could trigger alerts or additional security measures here
    }
});

// Custom middleware to log all auth attempts (before rate limiting)
const logAuthAttempts = (req, res, next) => {
    if (req.headers.authorization) {
        // Log auth attempt but avoid logging sensitive data
        const key = req.ip || 'unknown-ip';
        console.info(`AUTH ATTEMPT: IP: ${key}, Endpoint: ${req.method} ${req.originalUrl}, Time: ${new Date().toISOString()}`);
    }
    next();
};
...
export const protect = (req, res, next) => {
    ...
    logAuthAttempts(req, res, () => {
        // Then apply the rate limiter
